...
{
    public partial class AddProductsInBarComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            productsinbar = new Caixa.Models.SqlProjectFinal.ProductsInBar(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Caixa.Models.SqlProjectFinal.ProductsInBar args)
        {
            try
            {
                var sqlProjectFinalCreateProductsInBarResult = await SqlProjectFinal.CreateProductsInBar(productsInBar:productsinbar);
                DialogService.Close(productsinbar);
            }
            catch (System.Exception sqlProjectFinalCreateProductsInBarException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new ProductsInBar!" });
            }
        }
