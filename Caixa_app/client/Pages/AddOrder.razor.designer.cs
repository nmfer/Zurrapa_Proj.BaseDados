...
namespace Caixa.Pages
{
    public partial class AddOrderComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            order = new Caixa.Models.SqlProjectFinal.Order(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Caixa.Models.SqlProjectFinal.Order args)
        {
            try
            {
                var sqlProjectFinalCreateOrderResult = await SqlProjectFinal.CreateOrder(order:order);
                DialogService.Close(order);
            }
            catch (System.Exception sqlProjectFinalCreateOrderException)
            {
